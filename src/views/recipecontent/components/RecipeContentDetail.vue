  ...
  <el-card class="form-container" shadow="never">
    <el-form :model="recipe" :rules="loginRules" ref="recipeFrom" label-width="150px">
      <el-form-item label="内容" prop="nickName">
        <el-input v-model="recipe.content"></el-input>
      </el-form-item>
...
<script>
    ...
    import {insertRecipecontent,getByRecipecontent,updateRecipecontent} from '@/api/recipe/recipeContent'
    const defaultrecipe={
        username: '',
        pic:'',
    ...
    export default {
        ...
        created() {
            if (this.isEdit) {
                ...
                getByRecipecontent(this.$route.query.id).then(response => {
                ...
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        this.$confirm('是否提交数据', '提示', {
                            ...
                            if (this.isEdit) {
                                updateRecipecontent(this.$route.query.id, this.recipe).then(response => {
                                ...
                                console.log(1);
                                insertRecipecontent(this.recipe).then(response => {
                                    if(response.code==200) {
                                        this.$refs[formName].resetFields();
                                        this.recipe = Object.assign({}, defaultrecipe);
                                        this.$message({
